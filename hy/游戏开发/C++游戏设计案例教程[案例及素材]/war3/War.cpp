...
//	g_pSprBackGround=new Sprite(NULL,"resource\\background.bmp");
	g_pSprBackGround=new Sprite("resource\\background.jpg",TRUE);	//改为使用JPG图像
...
//	g_pSprFoemanAnima=new Sprite(NULL,"resource\\FoemanAnima.bmp");
	g_pSprFoemanAnima=new Sprite("resource\\FoemanAnima.gif");		//改为使用GIF图像
//	g_pSprFoemanAnima->InitAnima(1,6,150);

//	g_pSprGirlAnima=new Sprite(NULL,"resource\\GirlAnima.bmp");
	g_pSprGirlAnima=new Sprite("resource\\GirlAnima.gif");			//改为使用GIF图像
//	g_pSprGirlAnima->InitAnima(1,6,150);
	...
	/*下面是新增加的精灵对象*/
	g_pSprCharStateL=new Sprite(NULL,"resource\\CharState.bmp");	//人物状态框
	g_pSprCharStateR=new Sprite(NULL,"resource\\CharState.bmp");	

	g_pGirlSmall=new Sprite(NULL,"resource\\GirlSmall.bmp");		//人物小图片
	g_pFoemanSmall=new Sprite(NULL,"resource\\FoemanSmall.bmp");

	g_pSprWarBarTop=new Sprite("resource\\WarBarTop.png");			//顶部速度背景框
	
	g_pGirlSpeed=new Sprite("resource\\GirlSpeed.png");				//顶部速度小人
	g_pFoemanSpeed=new Sprite("resource\\FoemanSpeed.png");



	//将精灵添加到精灵管理对象	
	g_Sprm.AddSprite(g_pGirlSmall,3);
	g_Sprm.AddSprite(g_pFoemanSmall,3);
	g_Sprm.AddSprite(g_pSprWarBarTop,2);
	g_Sprm.AddSprite(g_pSprCharStateR,2);
	g_Sprm.AddSprite(g_pSprCharStateL,2);
	g_Sprm.AddSprite(g_pFoemanSpeed,3);
	g_Sprm.AddSprite(g_pGirlSpeed,3);
...
//	g_pSprGirlAnima->SetDrawAnimaInfo(400,280,0,TRUE);
	...
	g_pSprFoemanAnima->SetDrawCxImageInfo(200,240);	//改为CxImage的绘图参数设置
	g_pSprGirlAnima->SetDrawCxImageInfo(300,280);

	g_pSprWarBar->SetDrawInfo(0,445);

	/*下面是新增精灵对象的绘图参数设置*/
	g_pSprCharStateL->SetDrawInfo(0,460,TRUE,RGB(255,255,255));
	g_pSprCharStateR->SetDrawInfo(515,460,TRUE,RGB(255,255,255));
	g_pGirlSmall->SetDrawInfo(515,460);
	g_pFoemanSmall->SetDrawInfo(0,460);
	g_pSprWarBarTop->SetDrawCxImageInfo(0,0,800,100);

	//判断速度小人水平位置，并作相应改变
	if(g_nGirlSpeedX>730)
		g_nGirlSpeedX=50;
	else
		g_nGirlSpeedX+=8;

	if(g_nFoemanSpeedX>730)
		g_nFoemanSpeedX=50;
	else
		g_nFoemanSpeedX+=7;

	g_pGirlSpeed->SetDrawCxImageInfo(g_nGirlSpeedX,40);
	g_pFoemanSpeed->SetDrawCxImageInfo(g_nFoemanSpeedX,40);
